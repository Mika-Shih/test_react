...
function InputWithAddAndClearButton(props) {
  ...
  useEffect(() => {
    ...
    set_machine_data(newMachineData, () => {
      console.log(newMachineData.map((item) => item.sn));
      props.serial_number_get(newMachineData.map((item) => item.sn));
    });
  }, [props.serial_number, machine_data, props.serial_number_get]);
  useEffect(() => {
